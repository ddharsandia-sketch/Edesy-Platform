...
async function simulateSingleCall(
  agentPersona: string,
  callerPersona: string,
  maxTurns = 8
): Promise<{
  caller_persona_short: string
  turns: number
  conversation: Turn[]
  ...
  const callerMessages: OpenAI.ChatCompletionMessageParam[] = [
    {
      role: 'system',
  ...
  for (let turn = 0; turn < maxTurns; turn++) {
    ...
    if (turn >= 3 && endSignals.some(s => agentText.toLowerCase().includes(s))) break
...
export async function runSimulationBatch(
  ...
  const personas = Array.from({ length: numSimulations }, (_, i) =>
    ADVERSARIAL_PERSONAS[i % ADVERSARIAL_PERSONAS.length]
  )
  // Shuffle
  ...
  const results = await Promise.allSettled(
    personas.map(p => simulateSingleCall(agentPersona, p, maxTurns))
  )
  ...
  if (!valid.length) return { error: 'All simulations failed', results: [] }
  ...
  for (const r of valid) {
    for (const f of (r.grade?.failures ?? []) as string[]) {
...
export const simulationJobs = new Map<string, {
