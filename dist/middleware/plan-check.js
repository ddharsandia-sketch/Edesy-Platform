...
exports.checkCallLimits = checkCallLimits;
 ...
 *   }, ...)
 */
async function checkCallLimits(request, reply) {
    const { workspaceId } = request.user;
    const workspace = await prisma_1.prisma.workspace.findUnique({
        where: { id: workspaceId },
        select: { planTier: true }
    });
    const tier = workspace?.planTier || 'free';
    const limits = billing_1.PLAN_LIMITS[tier];
    ...
    // Check agent count
    const agentCount = await prisma_1.prisma.agent.count({ where: { workspaceId } });
